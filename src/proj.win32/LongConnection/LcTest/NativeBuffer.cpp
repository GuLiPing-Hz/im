...
BufferUnitSingle *ReadNativeBufferSingle(NativeBuffer *nativeBuf, int type, int &len) {
    ...
    ret->type = (BufferUnit::DataType) type;
    ...
    if (type == BufferUnit::type_char) {
        ...
        len = 2 + ret->str.length();//字符串长度+字符串
    } else {
...
VECBUNIT AutoParseNativeBuffer(NativeBuffer *nativeBuf) {
    ...
    do {
		...
		if (type > BufferUnit::type_array + 1) {//字符串解析不能放在这里
            ...
            arra->type = (BufferUnit::DataType) realType;//数组的具体类型
...
const NetworkUtil::DataBlock &NativeBuffer::getBuffer() {
    return data_;
}
...
bool NativeBuffer::writeString(const short len, const char *c) {
    writeType(len);//写入字符串长度
    return data_.append(c, len) > 0;//写入字符串
}

bool NativeBuffer::writeStringNoLen(const short len, const char *c) {
    return data_.append(c, len) > 0;//写入字符串
}
...
bool NativeBuffer::readString(short &len, char *c) {
...
template<typename T>
bool NativeBuffer::writeType(const T c) {
    int size = sizeof(c);
